...
@router.post("/api/sourcing/analyze/{finished_good_id}")
async def analyze_finished_good(finished_good_id: int):
    """Run the full sub-agent sourcing pipeline for one finished good."""
    try:
        proposal = await SourcingOrchestrator().run(finished_good_id)
        return proposal.model_dump()
    except ValueError as exc:
        return JSONResponse(status_code=404, content={"error": str(exc)})
    except Exception as exc:
        return JSONResponse(status_code=500, content={"error": f"Pipeline error: {exc}"})
